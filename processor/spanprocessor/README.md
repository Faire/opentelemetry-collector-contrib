...
The span processor modifies span data based on its name and attributes. Please refer to [config.go](./config.go) for the config spec.
...
- `name`: Modify the span name based on its attributes
- `status`: Modify the status of the span
- `attributes`: Normalize, modify, and extract attributes of the span, based on the semantic conventions (currently, only [db](https://github.com/open-telemetry/semantic-conventions/blob/main/docs/database/database-spans.md))
...
### Span attributes
Extracts and modifies attributes of the span in an opinionated fashion based on the [semantic conventions](https://github.com/open-telemetry/semantic-conventions/) of each span type, considering the configuration provided.

#### SQL Attributes
Extracts, normalizes, and sets attributes for SQL database spans based on the [SQL semantic conventions](https://github.com/open-telemetry/semantic-conventions/blob/main/docs/database/sql.md).

A span is considered a "SQL span" if it has a `db.system` attribute set to a value representing a database system known to use SQL as their primary query language, as listed in the semantic conventions.
- `collection_name`: specifies whether the processor should extract and set the table name as `db.collection.name` span attribute.
- `operation_name`: specifies whether the processor should extract and set the operation/procedure as `db.operation.name`.
```yaml
span:
  attributes:
    db:
      sql:
        ...
        collection_name: <true|false> # optional - default false
        operation_name: <true|false> # optional - default false
```
...
- `db.query.text`: `SELECT table.id FROM table WHERE table.id IN (?)`
